...
router.post('/checkout', validateToken, async (req, res) => {
    ...
    const priceId = plan === 'yearly'
        ? (isKrw ? process.env.STRIPE_PRICE_YEARLY_KRW : process.env.STRIPE_PRICE_YEARLY)
        : (isKrw ? process.env.STRIPE_PRICE_MONTHLY_KRW : process.env.STRIPE_PRICE_MONTHLY);

    if (!priceId) return res.status(500).json({ error: `Stripe price not configured for plan=${plan} currency=${currency}` });
