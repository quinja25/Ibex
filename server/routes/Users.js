...
const { Users, StudySessions, WaitlistEntries } = require('../models');
...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        // Award referral XP to waitlist referrer when they join the platform (fire-and-forget)
        try {
            const waitlistEntry = await WaitlistEntries.findOne({ where: { email: email.toLowerCase() } });
            if (waitlistEntry?.referredBy) {
                const referrerEntry = await WaitlistEntries.findOne({ where: { referralCode: waitlistEntry.referredBy } });
                if (referrerEntry) {
                    const referrerUser = await Users.findOne({ where: { email: referrerEntry.email } });
                    if (referrerUser) {
                        const PLATFORM_REFERRAL_XP = 100;
                        let newXp = referrerUser.xp + PLATFORM_REFERRAL_XP;
                        let newLevel = referrerUser.level;
                        while (newXp >= newLevel * 100) { newXp -= newLevel * 100; newLevel++; }
                        await referrerUser.update({ xp: newXp, level: newLevel });
                        console.log(`[Referral] Awarded ${PLATFORM_REFERRAL_XP} XP to ${referrerEntry.email} for referring ${email}`);
                    }
                }
            }
        } catch (e) { console.error('[Referral] Platform XP award failed:', e); }
...
// GET /users/waitlist-credits — return waitlist referral stats for the logged-in user
router.get('/waitlist-credits', validateToken, async (req, res) => {
    try {
        const entry = await WaitlistEntries.findOne({
            where: { email: req.user.email.toLowerCase() },
            attributes: ['waitlistCredits', 'referralCode'],
        });
        if (!entry) return res.json({ waitlistCredits: 0, referralCode: null, referralCount: 0 });

        const referralCount = entry.referralCode
            ? await WaitlistEntries.count({ where: { referredBy: entry.referralCode } })
            : 0;

        res.json({
            waitlistCredits: entry.waitlistCredits || 0,
            referralCode: entry.referralCode || null,
            referralCount,
        });
    } catch (err) {
        console.error('Waitlist credits error:', err);
        res.status(500).json({ error: 'Failed to fetch waitlist credits.' });
    }
});
