...
router.post('/chat', validateToken, async (req, res) => {
    try {
        ...
        if (!user.isPro && await enforceDailyQueryLimit(userId)) {
...
router.post('/upload-document', validateToken, upload.single('file'), async (req, res) => {
    try {
        ...
        // Free-tier: 3 document uploads max
        if (!req.user.isPro) {
            const docCount = await UserDocuments.count({ where: { userId } });
            if (docCount >= 3) {
                return res.status(403).json({
                    error: 'Free plan includes 3 document uploads. Upgrade to Pro for unlimited.',
                    limitReached: true,
                });
            }
        }
