...
const { Users, Reports, TrustEvents, StudySessions, WaitlistEntries } = require('../models');
...
// GET /admin/waitlist — export waitlist as CSV or JSON
router.get('/waitlist', async (req, res) => {
    try {
        const entries = await WaitlistEntries.findAll({
            order: [['createdAt', 'ASC']],
            attributes: ['id', 'name', 'email', 'country', 'role', 'referralCode', 'referredBy', 'waitlistCredits', 'createdAt'],
        });

        if (req.query.format === 'csv') {
            const header = 'id,name,email,country,role,referralCode,referredBy,waitlistCredits,joinedAt';
