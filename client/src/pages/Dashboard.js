...
import api from '../api';
import { useProStatus, ProBadge } from '../components/ProBadge';
import { startCheckout, openPortal } from '../utils/billing';
...
export const Dashboard = () => {
    ...
    const [waitlistCredits, setWaitlistCredits] = useState(null);
    ...
    useEffect(() => {
        ...
        api.get('/users/waitlist-credits').then(r => setWaitlistCredits(r.data)).catch(() => {});
        ...
        <div className="dash-page">
            ...
            <div className="dash-container">
                ...
                <div className="dash-tabs">
                    ...
                    <button className={`dash-tab ${activeTab === 'pro' ? 'active' : ''}`} onClick={() => setActiveTab('pro')}>
                        Pro {!proStatus.isPro && !proStatus.loading && <span className="dash-tab-pro-dot" />}
                    </button>
                </div>
                ...
                <div className="dash-tab-content">
                    ...
                    {activeTab === 'pro' && (
                        <div className="dash-pro-tab">
                            {proStatus.loading ? (
                                <p className="dash-pro-loading">Loading billing status…</p>
                            ) : proStatus.isPro ? (
                                <div className="dash-pro-active">
                                    <div className="dash-pro-active-header">
                                        <ProBadge style={{ fontSize: '0.85rem', padding: '5px 16px' }} />
                                        <h3>Pro Active</h3>
                                    </div>
                                    {proStatus.proExpiresAt && (
                                        <p className="dash-pro-expires">
                                            Renews {new Date(proStatus.proExpiresAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}
                                        </p>
                                    )}
                                    <ul className="dash-pro-features">
                                        <li>Unlimited AI queries per day</li>
                                        <li>Unlimited document uploads</li>
                                        <li>Conversational AI memory</li>
                                        <li>Spaced Repetition deck</li>
                                        <li>Weekly progress email</li>
                                    </ul>
                                    <button className="dash-btn dash-btn-outline" onClick={openPortal}>Manage Billing</button>
                                </div>
                            ) : (
                                <div className="dash-pro-upgrade">
                                    <h3>Unlock your full potential</h3>
                                    <p className="dash-pro-upgrade-sub">Upgrade to Pro for unlimited AI, your personal document vault, and Spaced Repetition.</p>
                                    <div className="dash-plan-controls">
                                        <div className="dash-plan-toggle">
                                            <button className={`dash-plan-btn${billingPlan === 'monthly' ? ' active' : ''}`} onClick={() => setBillingPlan('monthly')}>
                                                Monthly · {billingCurrency === 'krw' ? '₩9,900' : '$7'}
                                            </button>
                                            <button className={`dash-plan-btn${billingPlan === 'yearly' ? ' active' : ''}`} onClick={() => setBillingPlan('yearly')}>
                                                Yearly · {billingCurrency === 'krw' ? '₩79,000' : '$59'} <span className="dash-plan-save">-30%</span>
                                            </button>
                                        </div>
                                        <div className="dash-currency-toggle">
                                            <button className={`dash-currency-btn${billingCurrency === 'usd' ? ' active' : ''}`} onClick={() => setBillingCurrency('usd')}>USD $</button>
                                            <button className={`dash-currency-btn${billingCurrency === 'krw' ? ' active' : ''}`} onClick={() => setBillingCurrency('krw')}>KRW ₩</button>
                                        </div>
                                    </div>
                                    <ul className="dash-pro-features">
                                        <li>Unlimited AI queries per day</li>
                                    ...
                                    <button className="dash-btn dash-btn-primary" onClick={() => startCheckout(billingPlan, billingCurrency)}>
                                        Upgrade to Pro — {billingCurrency === 'krw'
                                            ? (billingPlan === 'monthly' ? '₩9,900/mo' : '₩79,000/yr')
                                            : (billingPlan === 'monthly' ? '$7/mo' : '$59/yr')}
                                    </button>
                                </div>
                            )}
                            {waitlistCredits && waitlistCredits.waitlistCredits > 0 && (
                                <div className="dash-waitlist-credits">
                                    <h4>Waitlist Referral Credits</h4>
                                    <p>{waitlistCredits.referralCount} friend{waitlistCredits.referralCount !== 1 ? 's' : ''} referred · <strong>{waitlistCredits.waitlistCredits} credits</strong> earned</p>
                                </div>
                            )}
                        </div>
                    )}
